...
import copy
log = core.getLogger()
link_count=12   # GLOBAL VARIABLE TO KEEP TRACK OF THE LINK COUNT  4SW=12, 8SW=28, 20SW=90
vts_tree={} #STORING THE TOPOS CREATED
vts_flag=0
...
#*******************************CODE CHANGES MADE******************
def _calc_spanning_tree_vts (dict_links):
  """
  Calculates the actual spanning tree BASED ON PROGRAMATICALLY REMOVED LINKS
...
def _handle_LinkEvent (event):
  ...
  if _prev[dp1][p1] is False:
    if _prev[dp2][p2] is False:
      ...
      return
  if(vts_flag==1 and _prev[dp1][p1] is True and _prev[dp2][p2] is True): #PUSH THE PORT_MOD ON SINGLE LINK FAILURE DETECTION
    if event.link.end in vts_tree:
      print("VTS will be called")
      tree_due_to_link_failure=vts_tree[event.link.end]
      start=time.time()
      update_tree_2(tree_due_to_link_failure)
      end=time.time()
      print("Time elapsed:",end-start)
  
  global link_count
  link_count-=1 #keeping count till last link
  if link_count==0:                                         #modified code
      log.debug("printing event.link: %s",event.link.end)
      _update_tree()
      log.debug("Spanning tree updated")          #modified code
      links_key=list(core.openflow_discovery.adjacency.keys())
      dict_links=core.openflow_discovery.adjacency    #GETTING LINKS DICTIONARY
      cp_dict_links=copy.deepcopy(dict_links)
      global vts_tree
      for i in links_key:
        del(cp_dict_links[i])
        if ((i.end not in vts_tree) and (reverse(i.end) not in vts_tree)): #TAKING DOWN LINKS ONE AT A TIME
          vts_tree[i.end]=_calc_spanning_tree_vts(cp_dict_links)
        cp_dict_links=copy.deepcopy(dict_links)
      print(vts_tree)
      global vts_flag
      vts_flag = 1
...
def _update_tree (force_dpid = None):
  ...
  log.debug("Spanning tree updated %s",tree)
...
def update_tree_2 (tree):  #THE CORRESPONDING VT IS TAKEN AS ARGUMENT AND PORT MOD IS SENT TO SWITCHES
  """
  Update spanning tree
